...
Стек: Java 17, Selenium WebDriver, JUnit 5, Maven

### Структура

- `src/test/java/org/example/` — автотесты
...
- Регистрация через UI упирается в капчу на production-окружении сайта. Автотест доходит до заполнения полей email/пароль и останавливается — задокументировано как ожидаемое ограничение при отсутствии доступа к тестовому окружению без капчи.
